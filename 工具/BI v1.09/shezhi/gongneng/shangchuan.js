...
const ShangchuanModule = (function() {
        ...
        firebase.database().ref('/fuzeren').once('value', (snapshot) => {
            ...
            for (let i = 1; i < data.length; i++) {
                ...
                if (!operatorId) {
                    ...
                    updates[`/fuzeren/${operatorId}/order`] = getNextOperatorOrder(existingData);
                    newCount++;
                }
                
                // 处理字段数据 - 直接写入数据，配置由firebase.js实时检查处理
    ...
    // 获取下一个负责人顺序值
    function getNextOperatorOrder(existingData) {
        const maxOrder = Math.max(...Object.values(existingData).map(d => d.order || 0), 0);
